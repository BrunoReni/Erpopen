...
export function BaixaContaReceberModal({ isOpen, onClose, onSuccess, conta }: BaixaContaReceberModalProps) {
  ...
  useEffect(() => {
    if (isOpen && conta) {
      ...
      const saldoReceber = conta.valor_original + (conta.juros || 0) - (conta.desconto || 0) - conta.valor_recebido;
      setValorReceber(saldoReceber);
      setJuros(0);
      setDesconto(0);
  ...
  const calcularValorFinal = () => {
    if (!conta) return 0;
    const saldoReceber = conta.valor_original + (conta.juros || 0) - (conta.desconto || 0) - conta.valor_recebido;
    return saldoReceber + juros - desconto;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (valorReceber <= 0) {
      setError('Valor a receber deve ser maior que zero');
      return;
    }

    const valorFinal = calcularValorFinal();
    if (valorReceber > valorFinal) {
      setError(`Valor a receber não pode ser maior que o saldo a receber (R$ ${valorFinal.toFixed(2)})`);
      return;
    }
    ...
    try {
      ...
      const baixaData = {
        ...
        data_recebimento: dataRecebimento ? new Date(dataRecebimento).toISOString() : null,
        observacoes: observacoes || null
      };

      await axios.post(
        `http://localhost:8000/financeiro/contas-receber/${conta.id}/baixar`,
        baixaData,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">Baixar Conta a Receber</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg space-y-2">
            ...
            <div className="text-sm text-gray-600">
              <span className="font-semibold">Cliente:</span> {conta.cliente_nome || 'N/A'}
            </div>
            <div className="text-sm text-gray-600">
          ...
          <div className="bg-green-50 p-4 rounded-lg space-y-2">
            ...
            <div className="flex justify-between text-sm border-t border-green-200 pt-2">
              <span className="text-gray-700 font-semibold">Saldo a Receber:</span>
              <span className="font-bold text-green-600">
                R$ {saldoReceber.toFixed(2)}
              </span>
            </div>
          </div>
