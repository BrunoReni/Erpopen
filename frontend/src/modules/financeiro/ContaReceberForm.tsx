...
export function ContaReceberForm({ isOpen, onClose, onSuccess, conta }: ContaReceberFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const url = conta 
        ? `http://localhost:8000/financeiro/contas-receber/${conta.id}`
        : 'http://localhost:8000/financeiro/contas-receber';
      ...
      setError(err.response?.data?.detail || 'Erro ao salvar conta a receber');
    } finally {
      setLoading(false);
    }
  };
